...
- **`detected / connected` counter on HUD** — show `len(_detected_ids) / len(_valid_blink_ids)` next to the fps pill while detection is running, e.g. `17 / 19 found`. Today the operator only sees raw counts after the run via the calibration log, and silent gaps (locked phones / backgrounded apps) hide that auto-stop will never fire. Glanceable mid-run progress, helps decide when to stop manually. One extra `cv2.putText` in `draw_hud`. Future extension: auto-stop after N seconds of no new detections.
- **Auto-write post-show report on detection-stop, not just on reset** — `_save_report` runs only from `reset_server()` (the R key). After the 17-phone show on 05 May the operator stopped detection with D and never pressed R, so the audience/engagement/game summary was lost (only the raw calibration log survived). Trigger the report whenever detection transitions from on→off via `toggle_detection`, the worker auto-stop, or the camera-lost path; dedupe so rapid D-toggles don't spam `debug/reports/`. Keep the existing reset-time write so the all-cleared snapshot still gets one.
...
- **Decode-fail back-off** — when a decoded ID fails the `_valid_blink_ids` check (TV reflection, screen in background, etc.), suppress decoding around that grid point for X seconds instead of just clearing it and re-decoding next cycle. Currently every false source consumes a full Manchester decode every `decode_interval` (0.2s); a 30s back-off turns repeat offenders into one decode per 30s. Biggest perf win in venues with active screens/monitors in frame. Optionally track repeat-offence count and back off harder each time, capped at ~5 min.
  - **Must clear all back-offs when `_valid_blink_ids` changes** — otherwise a phone that connects with a previously-rejected ID stays suppressed for X seconds.
  - Touches `blink_detector.py` (frozen zone) — needs careful integration with `_ever_active`, `decode_interval`, and the gated history recording. Validate against a debug capture from a venue with the problem (clean office tests will pass even if subtly broken). Worst-case failure mode is "real phone takes longer to detect" (recoverable via D toggle), not detector breakage.
